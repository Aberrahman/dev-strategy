...
class FourthStrategicLevel(models.Model):
    _name = 'strategy.fourthlevel'
    _description = '4ème Niveau stratégique'

    reference = fields.Char(string='Reference', default='New', readonly=True, required=True)
    name = fields.Char(required=True, string='4ème Niveau stratégique')
    level_id = fields.Char(required=True, string='ID 4ème Niveau')
    ...
    def create(self, vals):
        if vals.get('reference', 'New') == 'New':
            vals['reference'] = self.env['ir.sequence'].next_by_code('strategy.fourthlevel') or 'New'
        return super(FourthStrategicLevel, self).create(vals)
